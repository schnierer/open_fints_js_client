    ...
    me.MsgInitDialog = function(cb){
	   ...
	   me.SendMsgToDestination(msg,function(error,recvMsg){
			if(error){
				...
				// TODO
                var is1822direkt = me.blz === 50050222;// check if 1822direkt bank (PIN/TAN)

				// Prüfen ob Erfolgreich
				var HIRMG = null;
				try{HIRMG = recvMsg.selectSegByName("HIRMG")[0];}catch(e){};
                var hirmgCode = HIRMG.getEl(1).getEl(1);
				if(HIRMG!=null && (hirmgCode=="0010" || hirmgCode=="3060")){
					if(Helper.checkMsgsWithBelongToForId(recvMsg,HKVVB.nr,"0020",is1822direkt)){
					try{
